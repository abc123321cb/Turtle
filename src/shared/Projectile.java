...
public class Projectile implements Loopers {
    ...
    public Projectile(double x, double y, double xvel, double yvel, int chunkx, int chunky, boolean friendly, int lifespan, int damage, int imgindex, double angle){
        this.x = x;
        this.y = y;
        this.xvel = xvel;
        this.yvel = yvel;
    ...
    public boolean move(){
        ...
        hitboxWait --;
        if(hitboxWait <= 0) {
            new Hitbox(x + FinalOptions.CHUNKSIZE * (chunkx - Game.chunks[4].chunkx), y + FinalOptions.CHUNKSIZE * (chunky - Game.chunks[4].chunky)
    ...
    public void draw(Graphics g, int xoffset, int yoffset){
        ...
        g.drawImage(Utility.rotate(projectileimgs[imgindex], angle), x-xoffset,y-yoffset,10,10,null);
    }
}
