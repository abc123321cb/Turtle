...
public class Chunk {
    ...
    public void update(int colum, int row) {
        ...
        int index = -1;
        for (int i = 0; i < FinalOptions.TEXTUREGROUPS.size(); i++) {
            if (Arrays.equals(new int[] { area.lower_index, area.upper_index, (area.change) ? 1 : 0 },
                    FinalOptions.TEXTUREGROUPS.get(i))) {
                index = i;
                break;
            }
        }
        if (index != -1) {
            index++;
            index = (index == FinalOptions.TEXTUREGROUPS.size()) ? 0 : index;
            area.lower_index = FinalOptions.TEXTUREGROUPS.get(index)[0];
            area.upper_index = FinalOptions.TEXTUREGROUPS.get(index)[1];
            area.change = FinalOptions.TEXTUREGROUPS.get(index)[2] == 1;
    ...
    public void generateMap() {
        ...
        double globalflatness = 0.005 * scale;
        double temperaturescale = 0.005 * scale;
        double moisturescale = 0.005 * scale;
        double altitudescale = 0.005 * scale;

        OpenSimplexNoise heightsimplex = new 
        OpenSimplexNoise(generator.nextLong());
        OpenSimplexNoise finefeaturesimplex = new 
        OpenSimplexNoise(generator.nextLong());
        OpenSimplexNoise sharpfeaturesimplex = new 
        OpenSimplexNoise(generator.nextLong());
        OpenSimplexNoise localflatnesssimplex = new 
        OpenSimplexNoise(generator.nextLong());
        OpenSimplexNoise temperaturesimplex = new
        OpenSimplexNoise(generator.nextLong());
        OpenSimplexNoise moisturesimplex = new
        OpenSimplexNoise(generator.nextLong());
        OpenSimplexNoise altitudesimplex = new
        OpenSimplexNoise(generator.nextLong());


        for (int x = 0; x < FinalOptions.CHUNKSIZE; x++) {
            for (int y = 0; y < FinalOptions.CHUNKSIZE; y++) {
                int globalx = x + chunkx * FinalOptions.CHUNKSIZE;
